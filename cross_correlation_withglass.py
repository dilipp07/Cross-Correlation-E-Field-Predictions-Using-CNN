...
def load_electric_fields(i):
    ...
    with open(r"C:\\Users\\dilip\\OneDrive\\Desktop\\MTP\\artifacts\\E_wg\\char_E_wg{}.pkl".format(i), 'rb') as file1:
        
        E_t_wg = pickle.load(file1)
...
def calculate_cross_correlation_intensities(tau_values, E_t_wg, E_t_r, time_values):
    
   
    cross_correlation_intensities = np.zeros(len(tau_values))
    for j in range(len(tau_values)):
        tau = tau_values[j]
        shifted_electric_field = np.roll(E_t_wg, int(-tau / (time_values[1] - time_values[0])))
...
def main():
    ...
    for i in range(1, 1001):
        E_t_wg, E_t_r = load_electric_fields(i)
        time_values = load_time_values()
        tau_values = np.linspace(-75, 75, 1600) * 10**-14
        cross_correlation_intensities = calculate_cross_correlation_intensities(tau_values, E_t_wg, E_t_r, time_values)
        normalized_intensities = normalize_intensities(cross_correlation_intensities)
        filename = f"C:\\Users\\dilip\\OneDrive\\Desktop\\MTP\\artifacts\\cross-correlation_G\\cross-correlation_Glass{i}.pkl"
